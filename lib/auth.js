...
async function loginWithLocalServer(cfg, opts = {}) {
  ...
  // ️ 定时器必须可清除：否则登录成功后事件循环仍被这个 5 分钟定时器占用，
  //    进程会"卡在登录阶段"不退出（用户已认证成功却看不到命令行返回）。
  let timeoutId = null;
  const timeoutPromise = new Promise((_, rej) => {
    timeoutId = setTimeout(() => rej(new Error('等待登录超时（10 分钟）')), 600000);
  });
  let code;
  try {
    code = await Promise.race([codePromise, timeoutPromise]);
  } finally {
    if (timeoutId) clearTimeout(timeoutId);   // 无论成功/失败/超时都释放
    server.close();
  }
