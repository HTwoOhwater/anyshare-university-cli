...
async function loginWithPassword(cfg, opts = {}) {
  ...
  if (!signinRes.ok) {
    const detail = signinData.error_description || signinData.message || signinData.error || signinText.slice(0, 300);
    throw new Error(`登录失败: HTTP ${signinRes.status} ${detail}`);
  }
  const redirectUrl = signinData.redirect || signinData.redirect_url;
  if (!redirectUrl) {
    const detail = signinData.message || signinData.error_description || JSON.stringify(signinData).slice(0, 300);
    throw new Error(`登录未成功（未返回 redirect）: ${detail}\n提示：账号或密码错误，或该实例启用了二次验证/验证码`);
  }

  // 4) 跟随重定向直到拿到 code（不需要真正监听回调端口）
  let code = null;
  ...
  for (let hop = 0; hop < 10 && current; hop++) {
    ...
    if (!loc) {
      ...
      throw new Error(`未能获取授权码（第 ${hop + 1} 跳）: HTTP ${res.status} ${body.slice(0, 200)}`);
    }
